...
public final class ConfigCommand {
    ...
    private static ArgumentBuilder<CommandSourceStack, ?> buildNode(ConfigEntryImpl<?> cfg){
        return switch (cfg) {
        ...
        return Commands.literal(entry.key())
                ...
                .then(Commands.argument(entry.key(), StringArgumentType.word())
                        .suggests((ctx, b) -> SharedSuggestionProvider.suggest(
                                Arrays.stream(((Enum[]) enumClass.getEnumConstants())).map(e -> ((Enum) e).name()), b))
                        .executes(ctx -> {
    ...
    private static Class<?> inferElemClass(ConfigEntryImpl.ListEntry<?> entry){
        List<?> cur = entry.get();
        if (cur != null && !cur.isEmpty() && cur.get(0) != null) return cur.get(0).getClass();
        List<?> def = entry.defaultValue();
        if (def != null && !def.isEmpty() && def.get(0) != null) return def.get(0).getClass();
        return String.class;
    }
