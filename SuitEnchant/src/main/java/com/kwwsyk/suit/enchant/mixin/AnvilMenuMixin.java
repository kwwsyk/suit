...
import com.kwwsyk.suit.enchant.ench.EnchMerger;
...
import com.kwwsyk.suit.enchant.IAnvilMenuExtension;
...
public abstract class AnvilMenuMixin extends ItemCombinerMenuMixin implements IAnvilMenuExtension {
    ...
    @Unique
    private boolean suit$renamed = false;
    ...
    public void suit$rebuildAnvilMechanic(CallbackInfo ci) {
        ...
        long basicCost = 0L;
        suit$renamed = false;//int renameCost = 0; //Suit change: remove rename cost Keep it to check
...
//            } Now receive negative repair cost value
            ...
            if (!suit$renamed && suit$repairXpCost == 0) {
                suit$clearResults();
                ci.cancel();
                return;
            }
    ...
    private int suit$preventClearAfterRename(int i, @Local(ordinal = 1) int renameCost) {
        if (!ServerConfigs.DEBUG_ANVIL_ENCH_MERGE.get()) {
            return i;
        }
        suit$renamed = renameCost > 0;//also read renameCost to judge if rename is applied
        if (i <= 0 && (suit$renamed || suit$costXp.get() > 0)) {
            return 1;
        }
        return i;
    }
    ...
    public void suit$mayPickup(Player player, boolean hasStack, CallbackInfoReturnable<Boolean> cir){
        cir.setReturnValue(
                (player.hasInfiniteMaterials() || EnchUtil.transformLevelToXpCost(player.experienceLevel) >= this.suit$costXp.get())
                        && (this.suit$costXp.get() > 0 || suit$renamed)
        );
        cir.cancel();
    }
